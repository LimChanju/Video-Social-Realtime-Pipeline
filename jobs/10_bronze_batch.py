...
builder = (
    SparkSession.builder.appName("bronze_batch_snippet")
...
sample_df = spark.createDataFrame(sampled_rdd, schema=df.schema)
...
if Path(BRONZE).exists():
    try:# 최근 7일치 데이터만 필터링
        ...
        bf_video = bronze_recent.select("video_id").na.drop().agg(
            expr("bloom_filter(video_id, 100000, 0.01) as bf_video")
        ).collect()[0]["bf_video"]
